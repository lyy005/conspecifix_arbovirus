...
def makeImages(sp):
	...
	for spStrain in specialStrains:
		...
		for strainPair in sorted(strainMap.keys()):
			...
			for i in range(maxLen):
				...
				if (sum(mutations)) != 0:
					...
					hmRate = 50 				    # YY's distinguish identical sequences vs. really different sequences
		...
		fig, (ax0) = plt.subplots(1,1, figsize=(12, 10)) 		# YY's change
		...
		for i in range(9):
			...
			tickLabels.append(int((data.max()-1)*(i/8.0)*BUCKET_SIZE)/float(BUCKET_SIZE))  # YY's change
		labelThings = [mark+1 for mark in tickLabels]
		print tickLabels
		print data.max(),data.min()
		print labelThings
		cbar = fig.colorbar(c, ax=ax0, ticks = labelThings)
		#cbar.formatter = LogFormatterExponent(base=10) # 10 is the defaulti
