...
def makeImages(sp):
	...
	if not fromSave:  		## YY: if the mapOfRecombination.txt file does not exist
		strainMap = {}
		for s1 in allStrains:
			...
			for s2 in allStrains:
				if s1 == s2: 		
					continue
				name = "&&&".join([s1,s2])	 ## YY: YFV-MH633684_China_AngolaGenotype_HumanHomosapiens_2016.fa&&&YFV-KY495641_China_AngolaGenotype_HumanHomosapiens_03112016.fa and YFV-KY495641_China_AngolaGenotype_HumanHomosapiens_03112016.fa&&&YFV-MH633684_China_AngolaGenotype_HumanHomosapiens_2016.fa
			#strainMap[s1] = []
				strainMap[name] = []
...
#			print(BUCKET_SIZE)
			...
			for i in range(maxLen):
				...
				elif (sum(mutations) == 0) and (sum(recombs) == 0):		### YY's distinguish identical sequences vs. really different sequences
					hmRate = 50
...
#		plt.figure(figsize=(20,10)) # YY
		fig, (ax0) = plt.subplots(1,1, figsize=(12, 10)) # YY
#		fig, (ax0) = plt.subplots(1,1) # Brian
#		tickNames = [str(s) for s in np.arange(0,maxLen*BUCKET_SIZE,step = max(int(maxLen/10)/10,1))]  
		tickNames = [str(s) for s in np.arange(0,int(maxLen*BUCKET_SIZE/1000),step = round(float(maxLen*BUCKET_SIZE)/10000, 2))]	### YY's change: so the x-ticks are correct now
#		print ("###")
#		print (int(maxLen/10))
#		print (tickNames)
...
#		print labelThings
		...
		plt.tight_layout()									# YY's change increase the margin
